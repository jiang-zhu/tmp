...
#                     raise ValueError("track array contains NaNs")
                ...
                # release per-object working copies
                del object_slice, data_slice, grid_cell_area_slice
                del obj_mass_center, obj_track, obj_speed
                del obj_size, obj_min, obj_max, obj_mean, obj_tot
...
def BreakupObjects(
    ...
    del obj_structure_2D  # no longer needed after labelling
    TT = np.zeros((MaxOb))
    ...
    for obj in tqdm(range(len(rgiObjNrs))):
        ...
        if Av_2Dob[obj] <= AVmax:
            ...
            del TMP, rgiObjects2D_ACT, DATA_ACT
...
def MCStracking(
    ...
    #         stop()
        ...
        if (len(moving_averages) > 0) & (np.max(moving_averages) == 1):
            ...
            continue
        # free per-object working copies
        del pr_slice, bt_slice, pr_act, bt_act, bt_object_act
        del area_act, pr_max, pr_object_majoraxislen, bt_size, bt_min_temp
        del pr_object_act, TMP
...
def clean_up_objects(DATA,
    ...
    del object_indices  # free find_objects list before return
    gc.collect()
...
def moaap(
    ...
    MinTimeIVT = 12,                # minimum livetime of ARs [hours]
    ...
    StartDay = Time[0]
    SetupString = '-dt-'+str(dT)+'h_MOAAP-masks'
    NCfile = OutputFolder+DataName+'.ObjectMasks.'+str(StartDay.year)+str(StartDay.month).zfill(2)+SetupString+'.nc'
    ...
    if jet_test == 'yes':
        ...
        timer(start, end)
        del jet_objects_characteristics
        gc.collect()
    ...
    if ew_test == 'yes':
        ...
        del gr_mrg, gr_igw, gr_kelvin, gr_eig0, gr_er
        gc.collect()
    ...
    if front_test == 'yes':
        ...
        timer(start, end)
        del PV, Tgrad, Fstar, du, dv, vgrad, CoriolisPar
        gc.collect()
    ...
    if slp_test == 'yes':
        ...
        timer(start, end)
        del grCyclonesPT, grACyclonesPT
        gc.collect()
    ...
    if z500_test == 'yes':
        ...
        del cy_z500_objects_characteristics, acy_z500_objects_characteristics
        gc.collect()
...
#         MCSs.unit = ""
    ...
    dataset.close()
    print('Saved: '+NCfile)
    # Free any remaining large tracking arrays now that they are on disk
    _post_write_vars = ['jet_objects', 'FR_objects', 'CY_objects',
                        'AR_obj', 'MS_objects', 'IVT', 'VapTrans',
                        'Frontal_Diagnostic', 'uv200', 'TC_Tracks']
    _lcl = locals()
    for _v in _post_write_vars:
        if _v in _lcl:
            del _lcl[_v]
    gc.collect()
    import time
